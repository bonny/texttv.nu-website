<!doctype html>
<html>

	<head>
		<title>Text TV-app för Android</title>
		<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/foundation/6.2.3/foundation.min.css">
		<meta charset="utf-8">
		<link rel="canonical" href="https://texttv.nu/android/">
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<meta name="viewport" content="width=device-width, initial-scale=1.0">
		<style>
			.logo {
				margin-top: 20px;
				max-width: 80px;
				display: inline-block;
			}
		</style>
		<script>
		  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
		  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
		  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
		  })(window,document,'script','//www.google-analytics.com/analytics.js','ga');
		  ga('create', 'UA-181460-18', 'auto');
		  ga('require', 'linkid', 'linkid.js');
		  ga('send', 'pageview');	
		</script>
	</head>
	
	<body>
	
		<div class="row">
		
			<div class="column">

				<p class="text-center"><img class="logo" src="/images/text-tv-nu-logotyp.svg"></p>
											
				<h1 class="text-center">Text TV-app för Android</h1>
				
				<p>
					Med <a href="https://play.google.com/store/apps/details?id=com.mufflify.TextTVnu&hl=sv">text-tv-appen från texttv.nu</a>
					så får du nyheter och sportresultat snabb och enkelt rakt in i din android-telefon.
				</p>

				<p class="text-center">
					<a 
						href="https://play.google.com/store/apps/details?id=com.mufflify.TextTVnu&hl=sv" 
						class="button"
						>
							Ladda hem appen
						</a>
				</p>
				
				<div class="row">
			
					<div class="column small-12 medium-8">

						<h2 class="text-center">Funktioner i appen</h2>
						
						<ul>
							<li>Mobilanpassad text tv från grunden och upp</li>
							<li>Möjlighet att dela text tv-sidor till Twitter och Facebook, med skärmdump och permalänk</li>
							<li>Visa flera sidor samtidigt</li>
							<li>Favoritmärk dina favoritsidor och visa dom automatiskt när appen startas</li>
							<li>Få meddelande när det finns en uppdatering av sidan</li>
						</ul>

						<p class="text-center">
							<a href="https://play.google.com/store/apps/details?id=com.mufflify.TextTVnu&hl=sv" class="button">Ladda hem appen</a>
						</p>

					</div>

					<div class="column small-12 medium-4 text-center">
						<a href="https://play.google.com/store/apps/details?id=com.mufflify.TextTVnu&hl=sv">
							<img 
								src="/android/text-tv-android-app-screenshot.png" 
								alt="Skärmdump som visar hur sidan 100 ser ut i Androidappen för text tv"
								>
						</a>
					</div>			

				</div>
				
			</div>

		</div>		
	
	</body>

</html>