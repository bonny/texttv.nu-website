...
- [ ] **K1 — Reflekterad XSS via `jsoncallback`.** `views/api.php:139-145`. `get_html`-grenen sätter aldrig
      ...
      **Fix:** vitlista callbacken (`^[A-Za-z0-9_.]{1,64}$`) + `set_content_type("application/javascript")`.
      Helst: ta bort JSONP helt — apparna använder CORS (`ACAO: *`).
- [ ] **K2 — Fatalt fel dumpar DB-lösenordet till webbroten.** `config/config.php:6-44`. Shutdown-handlern
      ...
      en rättighetsändring från publik kreddläcka. **Fix:** ta bort hela `register_shutdown_function`-blocket.
- [ ] **K3 — `display_errors` + `db_debug` på i prod.** `config/database.php:66,86`. `/api/updated/abc/0`
      ...
      **Fix:** `db_debug = FALSE` på live + `display_errors=Off` i php-fpm-poolen.
- [x] **K4 — Facebook page access token hårdkodad i publikt repo.** `controllers/fb.php:8,13`, repot är
      ...
      var `"yolo_verify"`. **Åtgärdat 2026-08-01:** hela `fb`-controllern och rutterna borttagna.
      **Kvarstår manuellt:** återkalla token hos Meta — den ligger kvar i git-historiken och ska betraktas
      som komprometterad oavsett om den redan är utgången.
...
2. K1 — enda aktivt utnyttjbara hålet.
3. K2 + K3 — samma commit; K3 fixar också den trasiga JSON:en mot apparna.
4. M6 — CSP + `X-Frame-Options`, begränsa `ACAO: *` till `/api/*`.
5. M1 + M2 — rate limiting.
